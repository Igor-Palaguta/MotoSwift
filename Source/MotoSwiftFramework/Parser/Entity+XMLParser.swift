...
extension Entity {
   init(xml: XMLIndexer) throws {
      ...
      self.attributes = try xml.children
         .flatMap { $0.element?.name == "attribute" ? try Attribute(xml: $0) : nil }

      self.relationships = try xml.children
         .flatMap { $0.element?.name == "relationship" ? try Relationship(xml: $0) : nil }

      self.fetchedProperties = try xml.children
         .flatMap { $0.element?.name == "fetchedProperty" ? try FetchedProperty(xml: $0) : nil }
